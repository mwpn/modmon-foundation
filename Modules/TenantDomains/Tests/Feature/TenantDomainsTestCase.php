...
abstract class TenantDomainsTestCase extends TestCase
{
    use RefreshDatabase;
    ...
    protected function installStack(): ModuleManager
    {
        ...
        $result = $manager->install('tenant-domains');
        if (! $result['success']) {
            $this->fail('TenantDomains could not be installed: '.implode(' | ', $result['messages']));
        }
