...
<body class="h-full dark:bg-gray-900" x-data x-cloak>
    <div class="min-h-screen xl:flex">
        {{-- Backdrop (TailAdmin pattern) --}}
        <div
        ...
        {{-- Sidebar --}}
        <aside
            class="fixed top-0 left-0 z-99999 mt-0 flex h-screen flex-col border-r border-gray-200 bg-white px-5 text-gray-900 transition-all duration-300 ease-in-out dark:border-gray-800 dark:bg-gray-900"
            :class="{
        ...
        >
            <div
                class="flex items-center pt-8 pb-7"
        ...
        >
            ...
            >
                <div class="flex w-full grow flex-col items-center justify-between xl:flex-row xl:px-6">
                    {{-- Row 1: TailAdmin mobile topbar — hamburger | brand | dots --}}
                    <div class="flex w-full items-center justify-between gap-2 border-b border-gray-200 px-3 py-3 dark:border-gray-800 sm:gap-4 xl:justify-normal xl:border-b-0 xl:px-0 lg:py-4">
                        <button
                            type="button"
                            class="hidden h-11 w-11 flex-none items-center justify-center rounded-lg border border-gray-200 text-gray-500 dark:border-gray-800 dark:text-gray-400 xl:flex"
                        ...
                        <button
                            type="button"
                            class="flex h-11 w-11 flex-none items-center justify-center rounded-lg text-gray-500 dark:text-gray-400 xl:hidden"
                        ...
                        >
                            ...
                            <svg x-show="$store.sidebar.isMobileOpen" class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" x-cloak>
                        ...
                        {{-- Brand mark on mobile (TailAdmin logo slot) --}}
                        <a href="{{ url('/') }}" class="inline-flex items-center gap-2 xl:hidden">
                            <span class="flex h-8 w-8 flex-none items-center justify-center rounded-lg bg-brand-500 text-sm font-bold text-white">
                                {{ strtoupper(substr(config('app.name', 'M'), 0, 1)) }}
                            </span>
                            <span class="text-lg font-semibold text-gray-900 dark:text-white">{{ config('app.name', 'ModMon') }}</span>
                        </a>

                        <button
                            type="button"
                            class="flex h-11 w-11 flex-none items-center justify-center rounded-lg text-gray-700 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 xl:hidden"
                        ...
                        {{-- Desktop page context only --}}
                        <div class="hidden min-w-0 flex-1 xl:block xl:pl-4">
                            @isset($header)
                                {{ $header }}
                            @else
                                <p class="truncate text-theme-sm font-medium text-gray-800 dark:text-white/90">{{ config('app.name', 'ModMon') }}</p>
                            @endisset
                        </div>
                    </div>

                    {{-- Row 2: actions — hidden on mobile until dots open; always on xl --}}
                    <div
                    ...
                    >
                        <div class="flex items-center gap-2">
                            <button
                                type="button"
                                class="relative flex h-11 w-11 flex-none items-center justify-center rounded-full border border-gray-200 bg-white text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white"
                        ...
                        @if($userLabel)
                            ...
                            >
                                <button
                                    type="button"
                                    class="flex flex-none items-center text-gray-700 dark:text-gray-400"
                                    @click="open = ! open"
                                    aria-haspopup="menu"
                                    :aria-expanded="open"
                                ...
                                >
                                    <span
                                        class="mr-3 block overflow-hidden rounded-full"
                                        style="width: 2.75rem; height: 2.75rem; min-width: 2.75rem; min-height: 2.75rem;"
                                    >
                                        <x-foundation::avatar :initial="$userInitial" size="md" class="!h-full !w-full" />
                                    </span>
                                    <span class="mr-1 block font-medium text-theme-sm" data-shell-user>{{ $userLabel }}</span>
                                    <svg
                                ...
                                >
                                    <div class="border-b border-gray-200 px-1 pb-3 dark:border-gray-800">
                                        <span class="block font-medium text-theme-sm text-gray-700 dark:text-gray-300" data-shell-user>{{ $userLabel }}</span>
                                    </div>

                                    @if($logoutUrl)
                                        <form method="POST" action="{{ $logoutUrl }}" class="pt-3">
                                            @csrf
                                            <button
                                                type="submit"
                                                class="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-theme-sm font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300"
                                                role="menuitem"
                                                data-shell-logout
                                            >
